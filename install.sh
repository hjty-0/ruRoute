...
#  https://github.com/hjty-0/ruRoute
...
apply_routing_xray() {
    ...
    # Добавляем DNS секцию — российские домены резолвятся через RU DNS
    local dns_config
    dns_config=$(jq -n '{
        servers: [
            {
    ...
    jq --slurpfile routing "$f_routing"        --argjson dns "$dns_config"        '.routing = $routing[0] | .dns = $dns' "$CONFIG_PATH" > "$tmp"
...
if [ "$RUROUTE_IS_SINGBOX" = "true" ]; then
    ...
    dns_config='{"servers":[{"address":"https://dns.google/dns-query","domains":["geosite:category-ru"],"skipFallback":true},"8.8.8.8","1.1.1.1"],"queryStrategy":"UseIPv4"}'
    jq --slurpfile routing "$f_r" --argjson dns "$dns_config"         '.routing = $routing[0] | .dns = $dns' "$RUROUTE_CONFIG_PATH" > "$tmp"
fi
...
# Только применяем rules — без рестарта сервиса (иначе бесконечный цикл)
...
main() {
    ...
    detect_outbound_tag
    download_lists
    # Пробуем собрать geosite.dat (если есть Go или можем установить)
    if command -v go &>/dev/null || command -v git &>/dev/null; then
        log_section "Сборка geosite.dat"
        if bash "$RUROUTE_DIR/build-geosite.sh" 2>/dev/null; then
            log_ok "geosite.dat собран — используем geosite:ruroute"
            GEOSITE_BUILT=true
        else
            log_warn "Сборка geosite.dat не удалась, используем обычный режим"
            GEOSITE_BUILT=false
        fi
    else
        GEOSITE_BUILT=false
    ...
    # Скачиваем build-geosite.sh
    curl -fsSL --max-time 30 "$REPO_RAW/build-geosite.sh"         -o "$RUROUTE_DIR/build-geosite.sh" 2>/dev/null &&         chmod +x "$RUROUTE_DIR/build-geosite.sh" || true
    print_summary
}
