...
REPO_RAW="https://raw.githubusercontent.com/hjty-0/ruRoute/main/ruRoute_7"
VERSION="1.0.0"
...
download_lists() {
    ...
    # 4. AntiZapret (с fallback на zapret-info)
    log_info "Скачиваю AntiZapret..."
    local antizapret_ok=false
    if curl -fsSL --max-time 120 --retry 2 --retry-delay 3 \
        "https://antizapret.prostovpn.org/domains-export.txt" \
        -o "$LISTS_DIR/domains-antizapret.txt" 2>/dev/null; then
        local c; c=$(grep -vc '^$' "$LISTS_DIR/domains-antizapret.txt")
        log_ok "AntiZapret: $c доменов"
        antizapret_ok=true
    fi
    if [[ "$antizapret_ok" == false ]]; then
        log_info "AntiZapret недоступен, пробую zapret-info..."
        if curl -fsSL --max-time 60 \
            "https://raw.githubusercontent.com/zapret-info/z-i/master/dump.csv" \
            -o "/tmp/zapret-dump.csv" 2>/dev/null; then
            cut -d';' -f2 /tmp/zapret-dump.csv | grep -v '^$' | grep '\.'  | \
                sort -u > "$LISTS_DIR/domains-antizapret.txt"
            rm -f /tmp/zapret-dump.csv
            local c; c=$(wc -l < "$LISTS_DIR/domains-antizapret.txt")
            log_ok "zapret-info (fallback): $c доменов"
        else
            log_warn "Оба источника недоступны, пропускаю"
