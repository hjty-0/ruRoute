...
download_lists() {
    ...
    # 4. community.antifilter.download — большая база заблокированных доменов
    log_info "Скачиваю community antifilter (domains)..."
    local blocked_ok=false

    # Источник 1 — community.antifilter.download (доступен из РФ)
    if curl -fsSL --max-time 60         "https://community.antifilter.download/list/domains.txt"         -o "$LISTS_DIR/domains-antizapret.txt" 2>/dev/null &&         [[ $(wc -l < "$LISTS_DIR/domains-antizapret.txt" 2>/dev/null) -gt 100 ]]; then
        # Убираем всё кроме доменов (формат ://. или просто домен)
        grep -oP "[a-zA-Z0-9._-]+\.[a-zA-Z]{2,}" "$LISTS_DIR/domains-antizapret.txt" |             sort -u > "$LISTS_DIR/domains-antizapret.tmp"
        mv "$LISTS_DIR/domains-antizapret.tmp" "$LISTS_DIR/domains-antizapret.txt"
        local c; c=$(wc -l < "$LISTS_DIR/domains-antizapret.txt")
        log_ok "community antifilter: $c доменов"
        blocked_ok=true
    fi

    # Источник 2 — antifilter.download основной
    if [[ "$blocked_ok" == false ]]; then
        log_info "Пробую antifilter.download..."
        if curl -fsSL --max-time 60             "https://antifilter.download/list/domains.lst"             -o "$LISTS_DIR/domains-antizapret.txt" 2>/dev/null; then
            local c; c=$(wc -l < "$LISTS_DIR/domains-antizapret.txt")
            log_ok "antifilter.download: $c доменов"
            blocked_ok=true
        fi
    fi

    # Источник 3 — itdoginfo outside (сайты которые заблокированы в РФ)
    if [[ "$blocked_ok" == false ]]; then
        log_info "Пробую itdoginfo outside-raw..."
        if curl -fsSL --max-time 60             "https://raw.githubusercontent.com/itdoginfo/allow-domains/main/Russia/outside-raw.lst"             -o "$LISTS_DIR/domains-antizapret.txt" 2>/dev/null; then
            local c; c=$(wc -l < "$LISTS_DIR/domains-antizapret.txt")
            log_ok "itdoginfo outside: $c доменов"
            blocked_ok=true
        fi
    fi

    if [[ "$blocked_ok" == false ]]; then
        log_warn "Все источники недоступны, пропускаю"
