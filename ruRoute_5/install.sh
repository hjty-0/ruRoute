...
REPO_RAW="https://raw.githubusercontent.com/hjty-0/ruRoute/main/ruRoute_2"
VERSION="1.0.0"
...
download_lists() {
    ...
    # 4. AntiZapret
    log_info "Скачиваю AntiZapret..."
    if curl -fsSL --max-time 60         "https://antizapret.prostovpn.org/domains-export.txt"         -o "$LISTS_DIR/domains-antizapret.txt" 2>/dev/null; then
        local c; c=$(grep -vc '^$' "$LISTS_DIR/domains-antizapret.txt")
        log_ok "AntiZapret: $c доменов"
    else
        log_warn "AntiZapret недоступен, пропускаю"
    ...
    # 6. coder-stump/russian-ori-list
    log_info "Скачиваю coder-stump/russian-ori-list..."
    if curl -fsSL --max-time 60 \
        "https://raw.githubusercontent.com/coder-stump/russian-ori-list/main/russia.lst" \
        -o "$LISTS_DIR/domains-coder.txt" 2>/dev/null; then
        local c; c=$(grep -vc '^$' "$LISTS_DIR/domains-coder.txt")
        log_ok "coder-stump: $c доменов"
    else
        log_warn "coder-stump недоступен, пропускаю"
