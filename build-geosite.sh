...
# Определяем путь к xray в зависимости от панели
if [[ -d "/usr/local/x-ui/bin" ]]; then
    XRAY_DIR="/usr/local/x-ui/bin"
elif [[ -d "/var/lib/marzban" ]]; then
    XRAY_DIR="/var/lib/marzban"
elif [[ -d "/opt/hiddify-manager" ]]; then
    XRAY_DIR="/opt/hiddify-manager/xray"
else
    XRAY_DIR="/usr/local/x-ui/bin"
fi
GO_VERSION="1.23.0"
